...
and writes SAME_AS edges to HydraDB using batched UNWIND execution.
...
def resolve_entities(client: GraphClient, force_heuristic: bool = False) -> int:
    """
    Runs blocking, adjudicates candidate pairs, and writes SAME_AS edges into HydraDB using batched UNWIND writes.
    ...
    for p1, p2, score in pairs:
        ...
        if score >= 95.0:
            same_as_rows.append({
                "aid": p1["id"],
                "aname": _sanitize(str(p1.get("name", ""))),
                "asrc": str(p1.get("source", "raw")).lower(),
                "bid": p2["id"],
                "bname": _sanitize(str(p2.get("name", ""))),
                "bsrc": str(p2.get("source", "raw")).lower(),
            })
        elif score >= 85.0:
    ...
    if not same_as_rows:
        logger.info("Entity resolution complete. Created 0 SAME_AS edges.")
        return 0
    ...
    try:
        same_as_count = client.run_batch(batch_cypher, same_as_rows, batch_size=200)
    except Exception as exc:
        logger.warning("Batched SAME_AS write error: %s", exc)
        same_as_count = 0
